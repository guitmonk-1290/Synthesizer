...
{
public:
	sound_Interface(wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
	...
	bool Create(wstring sOutputDevice, unsigned int nSampleRate = 44100, unsigned int nChannels = 1, unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_thread = thread(&sound_Interface::MainThread, this);
...
private:
	...
	{
		((sound_Interface*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
